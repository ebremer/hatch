...
public class TiffSaver implements Closeable {
  ...
  public void writeImage(byte[] buf, IFD ifd, int no, int pixelType, int x, int y, int w, int h, boolean last, Integer nChannels, boolean copyDirectly) throws FormatException, IOException {
    ...
    synchronized (this) {
      ...
      if (channelsAllSameSize && (ifd.getImageWidth() == w && ifd.getTileWidth() == w) || (tileHeight * tileWidth * nChannels * bytesPerPixel == buf.length)) {
...
/*
    ...
    if (ifd.get(IFD.SOFTWARE) == null) {
      ifd.putIFDValue(IFD.SOFTWARE, "Hatch 1.0.0 by Wing and Beak");
    }
